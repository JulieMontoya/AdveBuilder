  ...
  200wrap_off=&5FC4
  210get_visited=&5FCD
  220get_state_bit=&5FD0
  230clear_state_bit=&5FD3
  240set_state_bit=&5FD6
  250select_msg=&5FD9
  260select_sysmsg=&5FDC
  270test_avail=&5FDF
  280test_avail_X=&5FE2
  290init_game=&5FE5
  300select_room=&5FE8
  310art_light=&5FEB
  320disp_desc=&5FEE
  330disp_exits=&5FF1
  340list_objects=&5FF4
  350get_cmd=&5FF7
  360action_cmd=&5FFA
  370inventory=&5FFD
 1000CLS
 ...
 1320IFM%=2PROCin
 1330IFV%=34PROChelp
 1340IFV%=35PROCstroke
 1350IFV%=36PROCcall
 1360IFV%=37PROCknock
 1900IF?cmdbuf=81THENE%=10
 2000CALLaction_cmd
 2010IFV%>0ANDV%<12AND(D%=2ORD%=4ORD%=9ORD%=11)ANDFNb(5)=0PROCm(68):R%=emo?11:?&82=0
 2015IFV%>0ANDV%<12AND(D%=2ORD%=4ORD%=9ORD%=11)ANDFNb(5)ANDFNv(D%)=0PROCm(56):PRINT
 2020IFV%=13ANDN%=12ANDE%=0N%=13:PROCexam
 2030IFV%=14ANDN%=8ANDE%=0ANDgsc?1=2PROCm(12):gsc?1=3:PROCcb(12)
 2040IFV%=21ANDM%=35PROCdone(18)
 2050IFV%=21ANDM%=4PROCdone(19)
 2060IFV%=21ANDM%=17PROCdone(20)
 2070IFV%=21ANDM%=25PROCdone(21)
 2080IFloc?25>16AND(R%=9ANDFNv(15)ORR%=15ANDFNv(9))loc?25=6
 2090UNTILE%=10
 2100CALLwrap_off
 2110PRINT'"Do you wish to play again? (Y/n) :";
 2120INPUT""Z$
 2130IF(ASCZ$AND31)<>14GOTO1000
 2140END
 2800CALLwrap_off
 ...
 3415IFN%=10PROCsb(2)
 3420IFN%=13ANDR%=14ANDgsc?1<3PROCm(2+gsc?1):V%=0:E%=0
 3430IFN%=13ANDR%=14ANDgsc?1>2PROCm(2):V%=0:E%=0
 3440IFN%=21ANDFNa PROCm(gsc?21MOD3+27):V%=0:E%=0:gsc?21=gsc?21+1
 3450IFN%=22ANDR%=13PROCsb(22)
 3460ENDPROC
 3500DEFPROCdone(M%)
 ...
 4100DEFPROCmend
 4110IFN%<>10THENE%=7
 4120IFN%=10ANDFNa=0THENE%=2
 4130IFN%=10ANDFNa ANDFNb(10)=0AND(FNb(0)=0ORFNh(8)=0)ANDFNh(16)=0V%=21:M%=14
 4140IFN%=10ANDFNa ANDFNb(10)=0AND(FNb(0)=0ORFNh(8)=0)ANDFNh(16)V%=21:M%=15
 4150IFN%=10ANDFNa ANDFNb(10)=0ANDFNh(8)ANDFNb(8)ANDFNh(16)=0V%=21:M%=16
 4160IFN%=10ANDFNa ANDFNb(10)=0ANDFNh(8)ANDFNb(8)ANDFNh(16)PROCsb(10):loc?8=254:loc?16=254:V%=21:M%=17
 4170ENDPROC
 4200DEFPROCchew
 4210IFN%<>8THENE%=2
 4220IFN%=8ANDFNa=0THENE%=7
 4230IFN%=8ANDFNa PROCsb(8):V%=21:M%=13
 4240ENDPROC
 4300DEFPROCin
 4310IFFNa=0THENE%=7
 4320IFN%<>14THENE%=2
